...
import { recordFnDep, mapFn, getFnKey, delRunninFnKey, getFnCtxByObj, setKeyUse, recordValKeyDep, runFn, markFnKey } from '../../helpers/fndep';
...
export function createFnCtx(
  ...
  if (source) {
    ...
    if (upstreamFnCtx) {
      ...
      fnCtx.depKeys.forEach(key => setKeyUse(key, curFnKey, upstreamFnCtx.isAsync));
      ...
      fnCtx.depKeys.forEach(key => setKeyUse(key, curFnKey, false));
    }
